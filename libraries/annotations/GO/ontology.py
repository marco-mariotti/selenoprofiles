...
from annotations.GO.utils import db_cursor_iterator, rewrite_db_query_markers
...
class GOTerm(object):
    ...
    __slots__ = ("id", "name", "aliases", "tags", "ontology", "extra")
...
class GeneOntologyNX(Ontology):
    ...
    def __iter__(self):
        """
        iterate over ontology, and returns each go as ontology object
        """
        for go in self._goid_dict:
            yield self.get_term_by_id (go)
    ...
    def get_all_parents_ids(self, nam):
        '''
        given a GOTerm object, retrieve all parents, return ids in a list
        '''
        parents = map (lambda x: x.value, self.get_term_by_id (nam).tags['is_a'])
        level2 = parents [:]
        while True:
            level1 = []
            for dad in map (lambda x: self.get_term_by_id (x), level2):
                try:
                    level1  += map (lambda x: x.value, dad.tags['is_a'])
                    parents  = list (set (level1[:] + parents))
                except KeyError:
                    continue
            if level1 == []:
                break
    ...
    def define_annotations_level (self, roots=['GO:0003674', 'GO:0005575', 'GO:0008150'], silent=False):
        '''
        parse DAG from given roots, and label each node with level number,
        number retained is maximum of possible.
        '''
        lower = []
        for root in roots:
            self.get_term_by_id (root).tags['level'] = 1
        level = 2
        while True:
            for term in set (roots):
                for child in self._internal_dag.predecessors (term):
                    lower.append (child)
                    self.get_term_by_id (child).tags['level'] = level
            level += 1
            if not silent:             print 'labelling nodes at level '+str (level)
            roots = lower [:]
            lower = []
            if len (roots) == 0:
                break
    ...
    def draw_GO_plus_parents (self, nam):
        '''
        draw ugly DAG from input node name, to first levels
        '''
        import matplotlib.pyplot as plt

        def format_coord(x, y):
            '''
            return GO description when mouse over node
            '''
            col = int(x*10+0.05)
            row = int(y-0.4)
            num = len (filter (lambda z: dicotab[z][1]==row, dicotab))
            for typ in filter (lambda z: dicotab[z][1]==row, dicotab):
                if col == int((dicotab[typ][0])*10):
                    return typ + '\n' + self.get_term_by_id(typ).tags['name'][0].value
            return filter (lambda z: dicotab[z]==[], dicotab)
        
        lala = list (set ([nam] + self.get_all_parents_ids(nam)))
        posit  = dict (map (lambda x: (x, [self.get_term_by_id(x).tags['level'], 0]), lala))
        dicotab = dict (map (lambda x: [x,(
            (float (filter (lambda y: posit[y][0] == posit[x][0], posit).index(x)+0.5) / \
             map (lambda y: posit[y][0], posit).count (posit[x][0])) \
            , -posit[x][0]) ], posit))
        fig = plt.figure(figsize=(8,8))
        self._nx.draw_networkx (self._internal_dag.subgraph(lala), dicotab,
                                node_size=1000,node_color='lightgreen',arrows=False,
                                edge_color='black',
                                node_shape=(((-0.04,-0.01),(-0.04,+0.01),(0.04,0.01),(0.04,-0.01)),0),
                                font_size=8, stlye=('solid', 'dotted')
                                )
        ax = fig.add_subplot(111)
        ax.hlines(range (-max (map (lambda x: x[0], posit.values())), 0), 0, 1, linestyles='dotted', alpha=0.5, colors='grey')
        ax.format_coord = format_coord
        ax.set_xticks([])
        ax.set_yticks(range (-max (map (lambda x: x[0], posit.values())), 0), range (max (map (lambda x: x[0], posit.values())),0, -1))
        ax.set_ylabel('GO level')
        ax.set_title ('GO graph for %s parents'%nam)
        plt.show()
